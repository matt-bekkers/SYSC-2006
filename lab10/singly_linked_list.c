...
{
    assert(head != NULL);
    intnode_t *insert_location = head;
    while(insert_location->next != NULL) {
        insert_location = insert_location->next;
    }

    intnode_t *newnode;
    for(newnode = other; newnode != NULL; newnode = newnode->next) {
        insert_location->next = malloc(sizeof(intnode_t));
        assert(insert_location->next != NULL);
        insert_location = insert_location->next;
        insert_location->value = newnode->value;
        insert_location->next = NULL;
    }
}
// Exercise 5
